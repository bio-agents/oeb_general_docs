...
Browsing Online
---------------
If you want to explore the benchmarking assessments of an analysis agent or scientific pipeline, the easiest way to do so it through the [OpenEBench website](https://openebench.bsc.es/scientific). At the web, you can choose the community of your interest and browse among the list of benchmarking events it has organized. Once selecting a particular event, the corresponding [summary table](1_explore_results.html#benchmarking-event-summary-table) gives a rundown of the participants' assessments.
...
This chart allows to visualize results from challenges that use two performance metrics (e.g precision vs recall), and apply several classification methods that transform them to tabular format, with a green color scale which makes it easier to find out which are the top-performing agents. These classification algorithms look for the optimization of the challenge metrics in order to group the agents according to their proximity to the 'ideal performance'.
...
This chart allows to visualize results from challenges that use one single performance metric (e.g F-Measure), and transform them to tabular format, with a green color scale which makes it easier to find out which are the top-performing agents.
...
The summary table condenses the results of a whole benchmarking event in a single table. Each of the columns corresponds to the quartiles/clusters of applying one of the classification methods described in the 2D ScatterPlot section, highlighted in green the top-performing agents. This view offers the possibility to see, at a glance, the overall results of a agent's performance across all the benchmarking challenges in a particular event.
